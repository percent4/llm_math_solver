...
# @file: data_generator_with_claude.py
# @time: 2024/8/13 16:02
import gradio as gr
import os
import re
import json
...
def first_turn_answer(query):
    ...
    print('*' * 50)
    messages = [{'role': 'user', 'content': get_system_prompt() + f"题目：{query}"}]
    ...
    #  get claude's result
    first_reply = result.content[0].text
    ...
    if '```python' in first_reply:
        ...
        second_reply = result.content[0].text
