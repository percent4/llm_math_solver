...
if __name__ == '__main__':
    ...
    # get numina-math-tir train dataset
    train_data = load_dataset('AI-MO/NuminaMath-TIR', split='train')
    dataset_with_index = train_data.map(add_index, with_indices=True)
    new_data = dataset_with_index.shuffle()
    # generate data using GPT-4o
    cnt = 0
    for i in range(len(new_data)):
        queId = new_data[i]['queId']
        if queId not in exist_file_list:
            problem = new_data[i]['problem']
            correct_answer = remove_boxed(last_boxed_only_string(new_data[i]['solution']))
            try:
